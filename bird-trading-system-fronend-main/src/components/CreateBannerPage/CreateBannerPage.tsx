...
import { ICreateBannerPageProps, ICreateBannerPageState, IBanner } from "./ICreateBanner";
...
import CreateBannerService from "./CreateBanner.service";
import { ImagePreview } from "../CommonComponents/Dialogs/ImagePreviewDialog";

export class CreateBannerPage extends React.Component<ICreateBannerPageProps, ICreateBannerPageState> {
    public service = new CreateBannerService();

    constructor(props: ICreateBannerPageProps) {
        super(props);
        this.state = {
            currentUser: null,
            address: '--',
            ...
            this.state.currentUser ?
                <div>
                    ...
                    >
                        ...
                        {/* <Divider orientation="left" className="app-divider-create-banner">
                            Thông tin banner
                        </Divider> */}
                        <Form
                            labelCol={{ span: 3 }}
                        ...
                        >
                            ...
                            <Form.Item label="Ngày Đăng Tin" required>
                                <DatePicker
                                    ...
                                    onChange={(value) => {
                                        this.setState({
                                            bannerDate: value?.toDate() ? value?.toDate() : null
                                        });
                                    }}
                                />
                                <div style={{ width: '97%', marginLeft: '3%', display: "flex" }}>
                                    {
                                        this.state.bannerRangeDate && this.state.bannerDate ?
                            ...
                            <Form.Item label="Gói Đăng" required>
                                <Select
                                    ...
                                    options={this.state.bannerPackage.map(item => {
                                        return {
                                            label: item.title,
                                            value: item.id
                                        }
                            ...
                            <Form.Item label="Thời gian đăng tin" style={{ marginBottom: 0 }}>
                                ...
                                ><strong>{`${this.state.bannerRangeDate} ngày`}</strong></span>
                            </Form.Item>
                            ...
                            <div className="app-group-button">
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        let key = `create_banner_${new Date().getTime()}`;
                                        this.props.openMessage('loading', 'Đang tạo bài đăng', undefined, key);
                                        this.onCreateBanner().then(res => {
                                            if (res.status) {
    ...
    private onCreateBanner(): Promise<any> {
        return new Promise(resolve => {
            let dataBanner: IBanner = {
                address: this.state.address,
                categoryId: this.state.productType,
                description: this.state.description,
                medias: this.getMediaFormSubmit(this.state.images),
                postTransaction: {
                    effectDate: this.formatEffectDate(this.state.bannerDate as Date),
                    expiredDay: this.state.bannerRangeDate,
            ...
            this.service.onCreateBanner(dataBanner).then((res: any) => {
                resolve(res);
            })
        })
    ...
    private getMessageExpiredDate(): React.ReactNode {
        let dateRange = cloneDeep(this.state.bannerRangeDate);
        let bannerDate = cloneDeep(this.state.bannerDate);
        let expiredDate = new Date(bannerDate?.setDate(bannerDate?.getDate() + dateRange) as number);
    ...
    private updateUnitPrice(packageId: string): void {
        let objPack = this.state.bannerPackage.filter((item: any) => {
    ...
    private getToMoney(): number {
        ...
        let bannerRangeDate = cloneDeep(this.state.bannerRangeDate);

        let totalMoney = unitPrice * bannerRangeDate;
        return totalMoney;
    }
    ...
    private disableBtnCreate(): boolean {
        let formGroup = {
            ...
            bannerDateRange: this.state.bannerRangeDate,
    ...
    private getBannerPrice(): void {
        // let bannerPrice = localStorage.getItem('bannerPrice');
        // let objPacks = JSON.parse(bannerPrice as string)
        // if (!CommonUtility.isNullOrUndefined(bannerPrice)) {
        //     this.setState({
        //         bannerPackage: objPacks,
        //         unitPrice: objPacks[0]?.price.price,
        //         packageId: objPacks[0]?.id,
        //     });
        //     return;
        // }
        this.service.getBannerPrice().then((res: any) => {
            if (res.status) {
                let arrayPacks = this.service.mappingStyleBannerPack(res.response.data);
                this.setState({
                    bannerPackage: arrayPacks,
                    unitPrice: arrayPacks[0]?.price.price,
                    packageId: arrayPacks[0]?.id,
                });
                localStorage.setItem('bannerPrice', JSON.stringify(arrayPacks));
                return;
            }
        ...
        this.service.getCategory().then((res: any) => {
            if (res.status) {
                this.setState({
                    ...
                    productType: res.response.data[0].id,
                });
                return;
            }
            let noti_id = `withdraw_${new Date().getTime()}`
